...
class rev:
    ...
    @commands.command(pass_context=True) #This takes a few tries to work! Also, if you have assigned yourself a second role in the "deniedroles" section, it may not work.
    async def status(self, ctx, *, game : str):
        """Changes the current game being played."""
        author = ctx.message.author
        for authorroles in list(self.settings["approvedroles"]):
            if str(authorroles) in list(self.settings["approvedroles"]):
                await self.bot.change_status(game = discord.Game(name=game), idle=False)
                await self.bot.send_message(ctx.message.channel, "{} The game has been changed.".format(author.mention))
            elif str(authorroles) in list(self.settings["deniedroles"]):
                print()
                print("Error: " + str(author) + " is missing permissions for the 'status' command.") #Allows you to see who used the command
            else:
                return
